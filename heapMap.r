...
ui = fluidPage(
  titlePanel("WRP Discharge Heatmap"),
               sidebarLayout(
                 sidebarPanel(
                 selectInput("season", "Season:",
                             c("Wet" = "wet", "Dry" = "dry")),
                 selectInput("rec", "Recreation:",
                             c("Kayak" = "kay", "Fish" = "fish")),
...
server <- function(input, output, session) {
  ...
  output$heat<-renderPlot({
    ...
    new = 0 
    fowGet <- function(name, species, num){
      actual <- paste(name, "p", sep = "")
      range <- flow[which(flow$Species_Label == species & flow$Node == name),]
      #print(nrow(range))
      ...
      if(as.symbol(actual) > range$Lower_Limit[1]){
        if(as.symbol(actual) < range$Upper_Limit[1]){
          print("Valid")
         #print(num)
          new <- 1
          return(new)
    ...
    F300WA <- fowGet("F300", "Willow Adult", 1)   
    F319WA <-fowGet("F319", "Willow Adult", 2)
    F34DWA <-fowGet("F34D", "Willow Adult", 3)
    F37BWA <- fowGet("F37B", "Willow Adult", 4)
    F45BWA <- fowGet("F45B", "Willow Adult", 5)
    F57CWA <- fowGet("F57C", "Willow Adult", 6)
    GLENWA<-fowGet("GLEN", "Willow Adult", 7)
    LA1WA <-fowGet("LA1", "Willow Adult", 8)
    LA11WA <-fowGet("LA11", "Willow Adult", 9)
    LA13WA <-fowGet("LA13", "Willow Adult", 10)
    LA14WA<-fowGet("LA14", "Willow Adult", 11)
    LA2WA<-fowGet("LA2", "Willow Adult", 12)
    LA20_2WA<-fowGet("LA20_2", "Willow Adult", 13)
    LA20WA<-fowGet("LA20", "Willow Adult", 14)
    LA3WA<-fowGet("LA3", "Willow Adult", 15)
    LA8WA<-fowGet("LA8", "Willow Adult", 16)
    second_c <- c(F300WA,F319WA,F34DWA, F37BWA, F45BWA, F57CWA,GLENWA, LA1WA,
                 LA11WA, LA13WA,LA14WA, LA2WA, LA20_2WA, LA20WA, LA3WA, LA8WA)
    
    
    F300TA <- fowGet("F300", "Typha Adult", 1)   
    F319TA <-fowGet("F319", "Typha Adult", 2)
    F34DTA <-fowGet("F34D", "Typha Adult", 3)
    F347BTA <- fowGet("F37B", "Typha Adult", 4)
    F45BTA <- fowGet("F45B", "Typha Adult", 5)
    F57CTA <- fowGet("F57C", "Typha Adult", 6)
    GLENTA<-fowGet("GLEN", "Typha Adult", 7)
    LA1TA <-fowGet("LA1", "Typha Adult", 8)
    LA11TA <-fowGet("LA11", "Typha Adult", 9)
    LA13TA <-fowGet("LA13", "Typha Adult", 10)
    LA14TA<-fowGet("LA14", "Typha Adult", 11)
    LA2TA<-fowGet("LA2", "Typha Adult", 12)
    LA20_2TA<-fowGet("LA20_2", "Typha Adult", 13)
    LA20TA<-fowGet("LA20", "Typha Adult", 14)
    LA3TA<-fowGet("LA3", "Typha Adult", 15)
    LA8TA<-fowGet("LA8", "Typha Adult", 16)
    
    
    third_c <- c(F300TA,F319TA,F34DTA, F347BTA, F45BTA, F57CTA,GLENTA, LA1TA,
                  LA11TA, LA13TA,LA14TA, LA2TA, LA20_2TA, LA20TA, LA3TA, LA8TA)
    
    F300MP <- fowGet("F300", "Steelhead Migration (Prolonged)", 1)   
    F319MP <-fowGet("F319", "Steelhead Migration (Prolonged)", 2)
    F34DMP <-fowGet("F34D", "Steelhead Migration (Prolonged)", 3)
    F347BMP <- fowGet("F37B", "Steelhead Migration (Prolonged)", 4)
    F45BMP <- fowGet("F45B", "Steelhead Migration (Prolonged)", 5)
    F57CMP <- fowGet("F57C", "Steelhead Migration (Prolonged)", 6)
    GLENMP<-fowGet("GLEN", "Steelhead Migration (Prolonged)", 7)
    LA1MP <-fowGet("LA1", "Steelhead Migration (Prolonged)", 8)
    LA11MP <-fowGet("LA11", "Steelhead Migration (Prolonged)", 9)
    LA13MP <-fowGet("LA13", "Steelhead Migration (Prolonged)", 10)
    LA14MP<-fowGet("LA14", "Steelhead Migration (Prolonged)", 11)
    LA2MP<-fowGet("LA2", "Steelhead Migration (Prolonged)", 12)
    LA20_2MP<-fowGet("LA20_2", "Steelhead Migration (Prolonged)", 13)
    LA20MP<-fowGet("LA20", "Steelhead Migration (Prolonged)", 14)
    LA3MP<-fowGet("LA3", "Steelhead Migration (Prolonged)", 15)
    LA8MP<-fowGet("LA8", "Steelhead Migration (Prolonged)", 16)
    
    
    fourth_c <- c(F300MP,F319MP,F34DMP, F347BMP, F45BMP, F57CMP,GLENMP, LA1MP,
                 LA11MP, LA13MP,LA14MP, LA2MP, LA20_2MP, LA20MP, LA3MP, LA8MP)
    
    F300SS <- fowGet("F300", "Steelhead Migration (Smolts)", 1)   
    F319SS <-fowGet("F319", "Steelhead Migration (Smolts)", 2)
    F34DSS <-fowGet("F34D", "Steelhead Migration (Smolts)", 3)
    F347BSS <- fowGet("F37B", "Steelhead Migration (Smolts)", 4)
    F45BSS <- fowGet("F45B", "Steelhead Migration (Smolts)", 5)
    F57CSS <- fowGet("F57C", "Steelhead Migration (Smolts)", 6)
    GLENSS<-fowGet("GLEN", "Steelhead Migration (Smolts)", 7)
    LA1SS <-fowGet("LA1", "Steelhead Migration (Smolts)", 8)
    LA11SS <-fowGet("LA11", "Steelhead Migration (Smolts)", 9)
    LA13SS <-fowGet("LA13", "Steelhead Migration (Smolts)", 10)
    LA14SS<-fowGet("LA14", "Steelhead Migration (Smolts)", 11)
    LA2SS<-fowGet("LA2", "Steelhead Migration (Smolts)", 12)
    LA20_2SS<-fowGet("LA20_2", "Steelhead Migration (Smolts)", 13)
    LA20SS<-fowGet("LA20", "Steelhead Migration (Smolts)", 14)
    LA3SS<-fowGet("LA3", "Steelhead Migration (Smolts)", 15)
    LA8SS<-fowGet("LA8", "Steelhead Migration (Smolts)", 16)
    
    fifth_c <- c(F300SS,F319SS,F34DSS, F347BSS, F45BSS, F57CSS,GLENSS, LA1SS,
                 LA11SS, LA13SS,LA14SS, LA2SS, LA20_2SS, LA20SS, LA3SS, LA8SS)
    
    F300SB <- fowGet("F300", "Steelhead Migration (Burst)", 1)   
    F319SB <-fowGet("F319", "Steelhead Migration (Burst)", 2)
    F34DSB <-fowGet("F34D", "Steelhead Migration (Burst)", 3)
    F347BSB <- fowGet("F37B", "Steelhead Migration (Burst)", 4)
    F45BSB <- fowGet("F45B", "Steelhead Migration (Burst)", 5)
    F57CSB <- fowGet("F57C", "Steelhead Migration (Burst)", 6)
    GLENSB<-fowGet("GLEN", "Steelhead Migration (Burst)", 7)
    LA1SB <-fowGet("LA1", "Steelhead Migration (Burst)", 8)
    LA11SB <-fowGet("LA11", "Steelhead Migration (Burst)", 9)
    LA13SB <-fowGet("LA13", "Steelhead Migration (Burst)", 10)
    LA14SB<-fowGet("LA14", "Steelhead Migration (Burst)", 11)
    LA2SB<-fowGet("LA2", "Steelhead Migration (Burst)", 12)
    LA20_2SB<-fowGet("LA20_2", "Steelhead Migration (Burst)", 13)
    LA20SB<-fowGet("LA20", "Steelhead Migration (Burst)", 14)
    LA3SB<-fowGet("LA3", "Steelhead Migration (Burst)", 15)
    LA8SB<-fowGet("LA8", "Steelhead Migration (Burst)", 16)
    
    sixth_c <- c(F300SB,F319SB,F34DSB,F347BSB, F45BSB, F57CSB,GLENSB, LA1SB,
                 LA11SB, LA13SB,LA14SB, LA2SB, LA20_2SB, LA20SB, LA3SB, LA8SB)
    
    F300AD <- fowGet("F300", "SAS Adult", 1)   
    F319AD <-fowGet("F319", "SAS Adult", 2)
    F34DAD <-fowGet("F34D", "SAS Adult", 3)
    F347BAD <- fowGet("F37B", "SAS Adult", 4)
    F45BAD <- fowGet("F45B", "SAS Adult", 5)
    F57CAD <- fowGet("F57C", "SAS Adult", 6)
    GLENAD<-fowGet("GLEN", "SAS Adult", 7)
    LA1AD <-fowGet("LA1", "SAS Adult", 8)
    LA11AD <-fowGet("LA11", "SAS Adult", 9)
    LA13AD <-fowGet("LA13", "SAS Adult", 10)
    LA14AD<-fowGet("LA14", "SAS Adult", 11)
    LA2AD<-fowGet("LA2", "SAS Adult", 12)
    LA20_2AD<-fowGet("LA20_2", "SAS Adult", 13)
    LA20AD<-fowGet("LA20", "SAS Adult", 14)
    LA3AD<-fowGet("LA3", "SAS Adult", 15)
    LA8AD<-fowGet("LA8", "SAS Adult", 16)
    ...
    F300KA <- fowGet("F300", "Rec. Use Kayak", 1)   
    F319KA <-fowGet("F319", "Rec. Use Kayak", 2)
    F34DKA <-fowGet("F34D", "Rec. Use Kayak", 3)
    F347BKA <- fowGet("F37B", "Rec. Use Kayak", 4)
    F45BKA <- fowGet("F45B", "Rec. Use Kayak", 5)
    F57CKA <- fowGet("F57C", "Rec. Use Kayak", 6)
    GLENKA<-fowGet("GLEN", "Rec. Use Kayak", 7)
    LA1KA <-fowGet("LA1", "Rec. Use Kayak", 8)
    LA11KA <-fowGet("LA11", "Rec. Use Kayak", 9)
    LA13KA <-fowGet("LA13", "Rec. Use Kayak", 10)
    LA14KA<-fowGet("LA14", "Rec. Use Kayak", 11)
    LA2KA<-fowGet("LA2", "Rec. Use Kayak", 12)
    LA20_2KA<-fowGet("LA20_2", "Rec. Use Kayak", 13)
    LA20KA<-fowGet("LA20", "Rec. Use Kayak", 14)
    LA3KA<-fowGet("LA3", "Rec. Use Kayak", 15)
    LA8KA<-fowGet("LA8", "Rec. Use Kayak", 16)
    
    eigth_c <- c(F300KA,F319KA,F34DKA,F347BKA, F45BKA, F57CKA,GLENKA, LA1KA,
                 LA11KA, LA13KA,LA14KA, LA2KA, LA20_2KA, LA20KA, LA3KA, LA8KA)
    
    F300FI <- fowGet("F300", "Rec. Use Fishing", 1)   
    F319FI <-fowGet("F319", "Rec. Use Fishing", 2)
    F34DFI <-fowGet("F34D", "Rec. Use Fishing", 3)
    F347BFI <- fowGet("F37B", "Rec. Use Fishing", 4)
    F45BFI <- fowGet("F45B", "Rec. Use Fishing", 5)
    F57CFI <- fowGet("F57C", "Rec. Use Fishing", 6)
    GLENFI<-fowGet("GLEN", "Rec. Use Fishing", 7)
    LA1FI <-fowGet("LA1", "Rec. Use Fishing", 8)
    LA11FI <-fowGet("LA11", "Rec. Use Fishing", 9)
    LA13FI <-fowGet("LA13", "Rec. Use Fishing", 10)
    LA14FI<-fowGet("LA14", "Rec. Use Fishing", 11)
    LA2FI<-fowGet("LA2", "Rec. Use Fishing", 12)
    LA20_2FI<-fowGet("LA20_2", "Rec. Use Fishing", 13)
    LA20FI<-fowGet("LA20", "Rec. Use Fishing", 14)
    LA3FI<-fowGet("LA3", "Rec. Use Fishing", 15)
    LA8FI<-fowGet("LA8", "Rec. Use Fishing", 16)
  
    
    ninth_c <- c(F300FI,F319FI,F34DFI,F347BFI, F45BFI, F57CFI,GLENFI, LA1FI,
                 LA11FI, LA13FI,LA14FI, LA2FI, LA20_2FI, LA20FI, LA3FI, LA8FI)
    
    F300CA <- fowGet("F300", "Cladophora Adult", 1)   
    F319CA <-fowGet("F319", "Cladophora Adult", 2)
    F34DCA <-fowGet("F34D", "Cladophora Adult", 3)
    F347BCA <- fowGet("F37B", "Cladophora Adult", 4)
    F45BCA <- fowGet("F45B", "Cladophora Adult", 5)
    F57CCA <- fowGet("F57C", "Cladophora Adult", 6)
    GLENCA<-fowGet("GLEN", "Cladophora Adult", 7)
    LA1CA <-fowGet("LA1", "Cladophora Adult", 8)
    LA11CA <-fowGet("LA11", "Cladophora Adult", 9)
    LA13CA <-fowGet("LA13", "Cladophora Adult", 10)
    LA14CA<-fowGet("LA14", "Cladophora Adult", 11)
    LA2CA<-fowGet("LA2", "Cladophora Adult", 12)
    LA20_2CA<-fowGet("LA20_2", "Cladophora Adult", 13)
    LA20CA<-fowGet("LA20", "Cladophora Adult", 14)
    LA3CA<-fowGet("LA3", "Cladophora Adult", 15)
